...
/// 규모는 매우 작지만 phone 에서 DB 를 SQL 을 사용하여 관리할 수 있도록
/// 여러가지 명령어를 제공한다
...
class TodoService {
  ...
  /// Future
  /// 함수(method)의 return type 으로 사용되는 키워드
  /// 이 함수는 언젠가 실행될 것이고, return 값이 null 이 아니다 라는 선언
  ...
  Future<List<Todo>> selectAll() async {
    final db = await database;
    final List<Map<String, dynamic>> todoList = await db.query("tbl_todolist");
    // .query("tbl_todolist", columns: ["id", "state,stime,count, complete"]);
